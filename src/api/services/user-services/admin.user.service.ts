...
export const adminUserService = {
    ...
    updateUser: async (id: string): Promise<UserUpdateRequestDto> => {
        const response =
            await api.patch<UserUpdateRequestDto>(`/api/admin/users/${id}`);
        return response.data;
    },

    deleteUser: async (id: string): Promise<void> => {
        await api.delete<void>(`/api/admin/users/${id}`);
    },

    getAllUsers: async (page: number = 0, size: number = 10): Promise<PageDto<AdminUserResponseDto>> => {
        const response =
            await api.get<PageDto<AdminUserResponseDto>>(`/api/admin/users?page=${page}&size=${size}`);
        return response.data;
    },

    getUserById: async (id: string): Promise<AdminUserResponseDto> => {
        const response =
            await api.get<AdminUserResponseDto>(`/api/admin/users/${id}`);
        return response.data;
    },
