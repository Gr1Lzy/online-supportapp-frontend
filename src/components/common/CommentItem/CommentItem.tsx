...
const CommentItem = ({ comment, currentUser, onEdit, onDelete, loading }: CommentItemProps) => {
    ...
    const handleEdit = (text: string) => {
        console.log("Attempting to edit comment with ID:", comment.id);
        console.log("New text:", text);

        if (!comment.id) {
            console.error("Cannot edit comment: Invalid or missing comment ID", comment);
            // Maybe show an error message to the user
            return;
        }

        // Only call edit if we have a valid ID
        onEdit(comment.id, text);
        setIsEditing(false);
    };

    const handleDelete = () => {
        // Make sure comment.id is valid before attempting to delete
        if (comment.id) {
            onDelete(comment.id);
            setShowDeleteConfirm(false);
        } else {
            console.error("Cannot delete comment - missing ID", comment);
        }
    };
